...
#### film_director
Таблица связи фильма и режиссера
...
### Эндпоинты
#### GET /directors Возвращает список всех режиссёров
#### GET /directors/{id} Возвращает режиссёра по id
#### DELETE /directors/{id} Удаляет режиссёра по id
#### POST /directors Создаёт режиссёра
```
{
"name": "director"
}
```
#### PUT /directors Обновляет данные у существующего режиссёра
```
{
"id": 1,
"name": "director"
...
Алгоритм умеет искать по подстроке. Например, при вводе подстроки «крад», возвращаются все фильмы содержащие в названии 
фильма или в имени режиссёра эту подстроку: «Крадущийся тигр, затаившийся дракон», «Крадущийся в ночи» и другие.
...
## add-feed - 4 points
### Описание
Выводит историю событий на платформе — добавление в друзья, удаление из друзей, лайки и отзывы, которые оставили 
друзья пользователя
### Эндпоинты
#### GET /users/{id}/feed
